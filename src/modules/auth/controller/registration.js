...
export const confirmEmail =asyncHandler(async(req,res,next)=>{
  ...
  if (!decoded?.id) {
  return next(new Error('in-valid decoded token' , {cause:400}))
  } else {
   // const user = await userModel.findOneAndUpdate({_id: decoded.id , confirmEmail:false},{confirmEmail:true})
    ...
    return res.status(200).redirect(process.env.LINK)
...
export const signin = asyncHandler(async(req,res,next)=>{
  ...
  if (!user) {
    ...
    return  next(new Error('email not confirm yet' , {cause:400}))
    } else {
    if (user.blocked) {
        return next(new Error('email blocked' , {cause:400}))
...
export const updatePassword = asyncHandler(async (req, res, next) => {
  ...
  if (updateUser.modifiedCount) {
    ...
    return next(new Error('failed update password', { cause: 400 }));
  }
});
...
export const forgetPassword = asyncHandler(async(req,res,next)=>{
  ...
  if (code == null) {
  return  next(new Error("in-valid code null not accepted"))
  } else {
...
export const refreshToken = asyncHandler(async(req,res,next)=>{ 
  ...
  return  next(new Error("invalid token payload"))
    ...
    if (!user) {
    return  next(new Error("account not register"))
    } else {
      if (user.confirmEmail) {
